...
SERPAPI_API_KEY = os.getenv("SERPAPI_API_KEY", "dc23befcdf07f4ca7d5fcb6af5e76a2fcf0384473e88be3dee29723d4e9c24c1"
)  ### FILL in here!!!!
...
# How many restaurants to fetch from Google Maps
RESTAURANT_SEARCH_LIMIT = 500  # Increase this for more coverage
...
# ==============================================================================
# FETCH ALL DC RESTAURANTS
# ==============================================================================


def get_all_dc_restaurants(client: ApiClient, limit: int = 500) -> list[dict]:
    """
    Fetch all DC restaurants from Outscraper.
    Returns list of dicts with name, place_id, google_id, etc.
    """
    print(f"\n{'='*50}")
    print(f"🔍 FETCHING ALL DC RESTAURANTS (limit: {limit})")
    print(f"{'='*50}")
...
def get_vibe_photos_serpapi(google_id: str, limit: int = 240) -> list[dict]:
    """
...
def download_photos(photos: list[dict], restaurant_name: str) -> list[str]:
    ...
    safe_name = "".join(
        c if c.isalnum() or c in " -" else "" for c in restaurant_name
    )
    ...
    for i, photo in enumerate(photos[:IMAGES_NEEDED]):
        ...
        try:
            ...
            if response.status_code == 200:
                filename = f"{safe_name}vibe{i+1}.jpg"
...
def main():
    ...
    if RESTAURANTS_FILE.exists():
        ...
        all_restaurants = get_all_dc_restaurants(client, limit=RESTAURANT_SEARCH_LIMIT)
