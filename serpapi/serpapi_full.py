...
def collect_images_for_restaurant(restaurant_name, search_terms, images_per_term):
    ...
    for term in search_terms:
        ...
        for url in image_urls:
            ...
            if url_hash in seen_url_hashes:
                print(f"    🔄 Skipping duplicate URL (already downloaded in different search)")
                total_duplicates += 1
                continue
            ...
            if download_image(url, save_path):
                ...
                try:
                    ...
                    if img_hash in seen_image_hashes:
                        print(f"    🔄 Duplicate image detected (same content, different URL) - removing")
                        os.remove(save_path)
                        total_duplicates += 1
                        continue
    ...
    print(f"  📊 Summary: {total_downloaded} unique images, {total_duplicates} duplicates removed")
...
def main():
    ...
    print(f"\n{'='*60}")
    print(f"🍽️  RESTAURANT IMAGE COLLECTION")
    print(f"{'='*60}")
